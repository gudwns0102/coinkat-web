      ...
      if(letterMove){
        return (
          <Motion defaultStyle={{x: 30, opacity: 1}} style={{x: spring(50), opacity: spring(0, option)}}>
            {value => 
              <img src={require('../assets/images/letter.png')} style={{position:'absolute', opacity: value.opacity, left: `${value.x}%`, top:250, height: 100, zIndex: 100}}/>
            }
          </Motion>
        )
      } else {
    ...
    const noti = (style) => {
      ...
      return (
        <Motion style={{opacity: spring(notiAppear ? 1 : 0), marginBottom: spring(notiAppear ? 50 : 0), width: spring(notiAppear ? 80 : 0)}}>
          {value => 
            <img src={require('../assets/images/bell.png')} style={{opacity: value.opacity, marginBottom: value.marginBottom, width: value.width, ...style}} />
          }
        </Motion>
      );
    }
    ...
    return(
      <div style={{...styles.container, ...this.props.style,}}>
        ...
        <div style={styles.imageWrapper}>
          <img src={require('../assets/images/server.png')} alt='Intro3' style={{width:'300'}}/>
          {letter()}
          <div style={{height:'100%', flex: 1, display:'flex', flexDirection:'column', alignItems:'flex-end', justifyContent:'space-around', marginRight:'10%'}}>
            <div style={{flex: 1, position:'relative', display:'flex', alignItems:'center', justifyContent:'center'}}>
              <img src={require('../assets/images/web.png')} style={{height: 200}} />
              {noti({position: 'absolute', top: '10%', right: -20, zIndex: 50})}
            </div>
            <div style={{flex: 1, position:'relative', display:'flex', alignItems:'center', justifyContent:'center'}}>
              <img src={require('../assets/images/mobile.png')} style={{height: 200}} />
              {noti({position: 'absolute', top: '10%', right: -20, zIndex: 50})}
            </div>
          </div>
        </div>
